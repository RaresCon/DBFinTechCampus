...
function Partner(props) {
    return (
        <div className=" flex w-16 flex-shrink-0 bg-white dark:bg-gray-700 shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border col-span-1 p-4">
            <img
                src={props.image_link} className="w-16 h-16"/>
        </div>
    )
}
...
function PartnerOffer(props) {
    ...
    const getOffer = () => {
        ...
        axios.post("http://localhost:8000/offers", send)
            .then(res => {
                console.log(res);
            })
            .catch(err => {
...
export default function OfferPartnerComponent(props) {
    ...
    return (
        <div className="">
            <div className={"p-16 px-32 " + (" ml-36")}>
                <div className="flex overflow-x-auto space-x-4">
                    {partners.map(item => <Partner image_link={item.image_link}/>)}
                </div>
                ...
                <div className="grid grid-cols-3 grid-rows-auto gap-4 mt-4">
                    ...
                    {offers.map(item => <PartnerOffer partner_name={item.partner_name} price={item.cost} num={item.num} category={item.category} description={item.description} _id={item._id}/>)}
